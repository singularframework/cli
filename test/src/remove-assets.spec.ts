...
describe('remove assets', function() {
  ...
  before('Suite preparation (setting up Singular project)', async function() {
    ...
    // Add assets to singular.json
    const singularJson = await fs.readJson(path.join(testDir, 'test', 'singular.json'));

    singularJson.project.assets.push('asset1.json', 'asset2.json', 'asset3.json');
  ...
  it('should unregister assets correctly', async function() {
    ...
    reporter.progress('Executing "sg remove assets asset1.json"');

    // Execute "sg remove assets asset1.json"
    await expect(cd('test').spawn('node', [sgPath, 'remove', 'assets', 'asset1.json']).promise)
    .to.eventually.have.property('code', 0);

    reporter.progress('Executing "sg remove assets asset2.json asset3.json"');

    // Execute "sg remove assets asset2.json asset3.json"
    await expect(cd('test').spawn('node', [sgPath, 'remove', 'assets', 'asset2.json', 'asset3.json']).promise)
    .to.eventually.have.property('code', 0);

    reporter.progress('Checking singular.json');

    // Check singular.json
    expect((await fs.readJson(path.join(testDir, 'test', 'singular.json'))).project.assets).to.be.empty;
  ...
  it('should refuse to register assets outside a Singular project', async function() {
    ...
    // Execute "sg remove assets asset1.json"
    reporter.progress('Executing "sg remove assets asset1.json" command');

    const child = spawn('node', [sgPath, 'remove', 'assets', 'asset1.json']);
    const errors: string[] = [];
